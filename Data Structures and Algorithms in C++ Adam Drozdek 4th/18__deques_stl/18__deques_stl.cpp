/*
deque (double-ended queue) - это список который позволяет получить доступ сразу с 2 концов.
Она сделана с ипоьзованием doubly linked list. Однако она еще имеет свойство доступа
элемента по индексу как в vector, при этом не имеет недостатка при добавлении нового
элемента, так как использует doubly linked list.
*/
